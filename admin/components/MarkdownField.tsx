...
import { useField, Component, Field, TextareaField } from "@contember/admin";

type MarkdownFieldProps = {
  field: string;
  label: string;
};
export const MarkdownField = Component<MarkdownFieldProps>(
  ({ field, label }) => {
    const accessor = useField<string>(field);

    // Here you can use any markdown editor
    return (
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <textarea
          ...
          onChange={(e) => {accessor.updateValue(e.target.value)}} 
        />
      </div>
    )
  },
